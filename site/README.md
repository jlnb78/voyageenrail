...
- `src/components/EuropeMap.astro` — the interactive map, ported from the
  ...
  curated or custom, kept in sync from the calculator's own script via the
...
- `src/lib/db-journeys.ts` — live "next real departures" lookup on
  ...
  (`iberie`, `nord`) don't have it, on purpose. For a custom "Autre trajet"
  search (below), it's wired to whatever the visitor typed.
- `/calculateur`'s **"Autre trajet" mode** — a real point-A-to-B search, not
  limited to the 9 curated routes: free-text départ/arrivée (with an
  optional correspondance), resolved live to real stations via
  `findStation()` in `db-journeys.ts`. The cost/CO2/duration estimate for an
  arbitrary pair is generated by `estimateRouteFromDistance()` in
  ...
  two points (`EuropeMap.astro`'s `evr-set-custom-line` event) — real
  geometry, not a curated `mapPts` path. A stopover, if given, is handled by
  two independent DB searches stitched together (`findJourneysWithVia`) —
...
- The `db-journeys.ts` live lookup couldn't be tested end-to-end from the
  ...
  free-text "Autre trajet" input resolves to the station a visitor actually
  meant (ambiguous names like "Paris" or "Cambridge" pick DB's top fuzzy
  match, which may not be the one intended).
- The "Autre trajet" cost/CO2 estimate is a straight-line-distance heuristic
